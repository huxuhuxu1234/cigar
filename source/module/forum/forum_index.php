...
$grids = array();
if($_G['setting']['grid']['showgrid'] || $gid == 0) {
	loadcache('grids');
	error_log('1');
...
// 	if(false) {
		...
		if ($gid == 0){
		    $grids['digest'] = C::t('forum_thread')->fetch_all_for_guide('digest', 0, array(), 1, 0, 0, 10, $_G['setting']['grid']['fids']);
...
// require DISCUZ_ROOT . 'data/template/1_1_common_header_forum_index.tpl.php';
// error_log('#   1101 : ' . DISCUZ_ROOT);
// include template('diy:forum/discuz:'.$gid);

// $list = print_r($grids,true);
// error_log($list);
// // error_log($_G['setting']['grid']['fids'] . '');
// exit('' . $gid);

//应该在这里输出页面信息      雪茄中国    hideDragon   2015/07/24.
if ($gid == 0){
    require DISCUZ_ROOT . 'data/template/cigar_forum_index.php';
}else{
    //其他板块继续调用默认的
    include template('diy:forum/discuz:'.$gid);
}
