...
class table_home_friend extends discuz_table
{
	...
	public function friend_data($uid){
	    
	    $t_m = DB::table('common_member_field_forum');
	    $t_f = DB::table($this->_table);

	    $sql = <<<EOT
SELECT f.uid,fuid,fusername,note,sightml FROM $t_f as f,$t_m as m
WHERE f.fuid = m.uid
AND f.uid = $uid
EOT;
	...
	public function is_friend($uid,$fuid){
	    $data = DB::fetch_first('SELECT count(1) as count FROM %t WHERE uid=%d AND fuid=%d',array($this->_table,$uid,$fuid));
