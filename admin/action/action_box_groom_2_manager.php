...
function add_data()
{
    ...
    //把数据保存到数据库
    C::t('cigar_box_groom_2')->insert($pid,$year,$rank);
...
function del_data(){
    ...
    echo C::t('cigar_box_groom_2')->delete_id_array($id_array);
...
function update_data()
{
    ...
    if (!record_exist('cigar_box_groom_2', $id)){
        exit('Parameter Error #5');
    }

    //把数据保存到数据库
    C::t('cigar_box_groom_2')->update($id,$rank);
    header('Location: box_groom_manager.php');
}
