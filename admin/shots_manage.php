<?php 
    ...
    $result = C::t('cigar_big_shots')->fetch_data_flag_admin();
    
    $s = C::t('cigar_stations')->fetch_data();
              ...
              <div class="col-md-12">
                      ...
                      <table  class="table table-datatable table-custom" id="basicDataTable">
                        <thead>
            				<tr>
            					...
            					<th class="sort-alpha">图片</th>
            					<th class="sort-alpha">名字</th>
            					<th class="sort-alpha">职位</th>
            					<th class="sort-alpha">雪茄</th>
            					<th >查看</th>
            				</tr>
            			</thead>
				...
				<?php 
				    ...
				    foreach ($result as $index => $row){
				        ...
				        $station = $row['station'];
				        $pid = $row['pid'];
				        $id_code = $salt->encode(2,$id);
				        echo <<<EOT
				<tr>
                    <td class="text-center "><input type="checkbox" id="$id"/></td>
                    <td>$i</td>
                    <td><img src="$img" width="100px" height="50px" /></td>
                    <td>$name</td>
                    <td>$station</td>
                    <td>$pid</td>
                    <td><a href="../big_shots_talk.php?id=$id_code" target="_blank">查看</a></td>
				</tr>
EOT;
				    }
				...
				?>
				    ...
				    <div class="modal fade" id="modalConfirm" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <form role="form" name="form_add" action="action.php?action=shots_manager&handle=add" method="post" enctype="multipart/form-data" parsley-validate >
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</button>
                          ...
                          <div class="modal-body">
                              ...
                              <div class="form-group">
                                <label for="passwordInput">名字：</label>
                                <input type="text" class="form-control" id="passwordInput" name="name" parsley-trigger="change" parsley-required="true" parsley-required-message="请输入名字"  >
                              </div>
                              
                              <div class="form-group">
        	                      <div class="form-group">
                                    <label for="input07" class="control-label">职位：</label>
                                    <select class="chosen-select form-control" name="station" id="input07">
                                        <?php 
                                        foreach ($s as $row){
...
EOT;
                              ...
                              <div class="form-group">
                                <label for="placeholderInput">雪茄：</label>
                                <input type="text" class="form-control" id="placeholderInput" name="cigar" parsley-required="true" parsley-trigger="change" parsley-required-message="请输入雪茄名字" > 
                              </div>
                    ...
                    <div class="modal fade" id="updateModalConfirm" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <form role="form" name="form_update" action="action.php?action=shots_manager&handle=update" method="post" enctype="multipart/form-data" parsley-validate >
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</button>
                          ...
                          <div class="modal-body">
                            ...
                            <div class="form-group">
                             ...
                             <div class="form-group">
                              ...
                              <div class="form-group">
        	                      <div class="form-group">
                                    <label for="input08" class="control-label">职位：</label>
                                    <select class="chosen-select form-control" name="station" id="input08">
                                        <?php 
                                        foreach ($s as $row){
...
EOT;
                              ...
                              <div class="form-group ">
		                        <label class="control-label">内容： </label>
		                        <div class="">
		                          <div class="form-control" id="input02" ></div>
		                        </div>
		                      </div>
...
//        }
       ...
       $("#basicDataTable tbody tr").dblclick(function(e){
         ...
         $.post('simple_query.php',{'info': 'update','type': 'shots_manager','id': id},function(data){
					...
					$('#updateModalConfirm input[name=cigar]').val(model.pid);
					$('#input02').code(model.content);
					$('#updateModalConfirm select').val(model.station);
					$('#updateModalConfirm select').trigger("chosen:updated");
					$('#pre_img').attr('src', '../' + model.img);
...
//			var id_array = new Array();
        ...
        $('#delete_confirm_btn').click(function(){
        	...
        	$.post('action.php?action=shots_manager&handle=del',{'id_array': id_array.join(',')},function(data){
...
//					alert('删除失败： ' + data);
      ...
      $('#input02').parent().find('textarea:eq(0)').attr('name',"content");

      $('form[name=form_update]').submit(function(){
      	$(this).find('textarea[name=content]').val($('#input02').code());
