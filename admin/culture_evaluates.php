<?php 
	...
	$result = C::t('cigar_culture_evaluates')->fetch_data_flag_admin();
              ...
              <div class="col-md-10">
                      ...
                      <table  class="table table-datatable table-custom" id="basicDataTable">
                        <thead>
							...
							<tr>
            					...
            					<th class="sort-alpha">文章标题</th>
            					<th class="sort-numeric">用户id</th>
            					<th class="sort-alpha">用户名字</th>
            					<th>是否点赞</th>
				...
				<?php 
				    foreach ($result as $index => $row){
				        $i = $index + 1;
				        $up_result = isset($row['is_up']) ? $row['is_up'] == 1 ? '是' : '否' : '否';
				        echo <<<EOT
<tr>
    <td class="text-center"><input type="checkbox" id="{$row['ceid']}"/></td>
    <td>$i</td>
    <td>{$row['title']}</td>
    <td>{$row['uid']}</td>
    <td>{$row['username']}</td>
...
EOT;
				...
				?>
                    ...
                    <div class="modal fade" id="updateModalConfirm" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <form role="form" name="form_add" action="action.php?action=culture_evaluates&handle=update" method="post" parsley-validate >
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</button>
                          ...
                          <div class="modal-body">
                              ...
                              <div class="form-group">
                                <label for="">uid：</label>
                                <input type="text" class="form-control" id="passwordInput" name="uid" readonly=""/>
                              </div>
                              <div class="form-group">
                                <label for="">文章标题：</label>
                                <input type="text" class="form-control" id="passwordInput" name="culture" readonly=""/>
                              </div>
                              <div class="form-group">
                                <label for="">是否点赞：</label>
                                <select class="chosen-select form-control" name="is_up" id="input07">
                              ...
                              <div class="form-group">
                                <label for="">评分：</label>
                                <input type="text" class="form-control" id="passwordInput" name="point" parsley-required="true" parsley-trigger="change" parsley-type="number" parsley-type-message="请输入正确的数字" parsley-required-message="请输入评分"/>
                              </div>
...
//          alert(id);
         ...
         $.post('simple_query.php',{'info': 'update','type': 'culture_evaluates','id': id},function(data){
...
// 					alert(data);
					...
					$('input[name=uid]').val(model.uid);
					$.post('simple_query.php',{'info': 'culture_name','cid': model.cid},function(data){
						$('input[name=culture]').val(data);
					});
					$.post('simple_query.php',{'info': 'uid','uid': model.uid},function(data){
						$('input[name=username]').val(data);
					});
					$('input[name=point]').val(model.point);
					$('select[name=is_up]').val(model.is_up);
...
//			var id_array = new Array();
        ...
        $('#delete_confirm_btn').click(function(){
        	...
        	$.post('action.php?action=culture_evaluates&handle=del',{'id_array': id_array.join(',')},function(data){
