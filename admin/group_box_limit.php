<?php 
	...
	$result = C::t('cigar_box_limit_group')->fetch_data();
	
	$group_data = C::t('common_usergroup')->fetch_all_by_not_groupid(C::t('cigar_box_limit_group')->usedGid());
                      ...
                      <table  class="table table-datatable table-custom" id="basicDataTable">
                        <thead>
            				<tr>
								...
								<th class="sort-alpha">组名字</th>
								<th class="sort-numeric">限制</th>
							</tr>
						...
						<?php 
						    ...
						    foreach ($result as $index => $row){
						    	$i = $index + 1;
						        $id = $row['groupid'];
						        $gName = C::t('common_usergroup')->fetch($id)['grouptitle'];
...
EOT;
				    ...
				    <div class="modal fade" id="modalConfirm" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <form role="form" name="form_add" action="action.php?action=group_limit&handle=add" method="post" enctype="multipart/form-data" parsley-validate >
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</button>
                          ...
                          <div class="modal-body">
                             ...
                             <div class="form-group">
                                <label for="input07" class="control-label">用户组名字：</label>
                                <select class="chosen-select form-control" name="groupid" id="input07">
                                    <?php 
                                    foreach ($group_data as $row){
                                        
                                        echo <<<EOT
<option value="{$row['groupid']}">{$row['grouptitle']}</option>
EOT;
                              ...
                              <div class="form-group">
                                <label for="passwordInput">限制：</label>
                                <input type="text" class="form-control" id="passwordInput" name="limit" parsley-trigger="change" parsley-required="true"  parsley-required-message="请输入限制"/>
                              </div>
                    ...
                    <div class="modal fade" id="updateModalConfirm" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <form role="form" name="form_add" action="action.php?action=group_limit&handle=update" method="post" enctype="multipart/form-data" parsley-validate >
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</button>
                          ...
                          <div class="modal-body">
                         	 ...
                         	 <div class="form-group">
                                <label for="passwordInput">用户组名字：</label>
                                <input type="text" class="form-control" id="passwordInput" readonly="" name="gName" parsley-trigger="change" parsley-required="true"  parsley-required-message="请输入用户组名字"/>
                              </div>
...
//             oTable01.$('tr.row_selected').removeClass('row_selected');
       ...
       $("#basicDataTable tbody tr").dblclick(function(e){
         ...
         var gName = $(this).find('td:eq(2)').text();
         
         $.post('simple_query.php',{'info': 'update','type': 'group_limit','id': id},function(data){
					...
					$('#updateModalConfirm input[name=gName]').val(gName);
					$('#updateModalConfirm input[name=limit]').val(model.limit);
...
//			var id_array = new Array();
        ...
        $('#delete_confirm_btn').click(function(){
        	...
        	$.post('action.php?action=group_limit&handle=del',{'id_array': id_array.join(',')},function(data){
