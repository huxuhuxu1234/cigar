<?php 
	...
	$result = C::t('cigar_suggestions')->fetch_data();
      ...
      <div class="row">
        ...
        <div id="content" class="col-md-12">
          ...
          <div class="pageheader">
            ...
            <h2><i class="fa fa-align-right"></i> 联系我们
            <span>意见管理</span></h2>
                  ...
                  <div class="tile-body">
                    ...
                    <div class="table-responsive">
                      <table  class="table table-datatable table-custom" id="basicDataTable">
						...
						<?php 
						    //id,uid,name,email,title,content,publish_time
						    foreach ($result as $index => $row){
						        ...
						        if ($uid == 0){
						            ...
						            if($result && isset($result[$uid])){
						                $uid_html = '<a href="../home.php?mod=space&uid='.$uid.'" target="_blank" >'.$result[$uid].'</a>';
						            }else{
						                $uid_html = '无';
						            }
						        }
...
EOT;
				    ...
				    <div class="modal fade" id="modalConfirm" tabindex="-1" role="dialog" aria-labelledby="modalConfirmLabel" aria-hidden="true">
                      <div class="modal-dialog">
                        <div class="modal-content">
                          <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">Close</button>
                            <h3 class="modal-title" id="modalConfirmLabel">查看</h3>
                          </div>
                          <div class="modal-body">
                              ...
                              <div class="form-group">
                                <label for="">名字：</label>
                                <input type="text" class="form-control" id="passwordInput" name="name" />
                              </div>
                              
                              <div class="form-group">
                                <label for="">E-mail：</label>
                                <input type="text" class="form-control" id="passwordInput" name="email" />
                              </div>
                              
                              <div class="form-group">
                                <label for="">标题：</label>
                                <input type="text" class="form-control" id="passwordInput" name="title" />
                              </div>
                              
                              <div class="form-group">
                                <label for="">提交时间：</label>
                                <input type="text" class="form-control" id="passwordInput" name="publish_time" />
                              </div>
                              
                              <div class="form-group">
                                <label for="">内容：</label>
                                <textarea class="form-control" id="input05" rows="6"  name="content"></textarea>
                              </div>
                          ...
                          <div class="modal-footer">
                            ...
                            <button class="btn btn-green" type="button" data-dismiss="modal">确定</button>
                          </div>
                        </div><!-- /.modal-content -->
                      </div><!-- /.modal-dialog -->
                    </div><!-- /.modal -->           
    ...
    <a href="#modalConfirm" data-toggle="modal" id="open_update_modal"> <span></span></a>
...
//             oTable01.$('tr.row_selected').removeClass('row_selected');
       ...
       $("#basicDataTable tbody tr").dblclick(function(e){
           ...
           $.post('simple_query.php',{'info': 'update','type': 'suggestions_manager','id': id},function(data){
  					...
  					// ok ，整理form
  					var model = eval('(' + data + ')');
  					$('#modalConfirm input[name=name]').val(model.name);
  					$('#modalConfirm input[name=email]').val(model.email);
  					$('#modalConfirm input[name=title]').val(model.title);
  					$('#modalConfirm textarea[name=content]').val(model.content);
  					$('#modalConfirm input[name=publish_time]').val(model.publish_time);
...
//			var id_array = new Array();
        ...
        $('#delete_confirm_btn').click(function(){
        	...
        	$.post('action.php?action=suggestions_manager&handle=del',{'id_array': id_array.join(',')},function(data){
